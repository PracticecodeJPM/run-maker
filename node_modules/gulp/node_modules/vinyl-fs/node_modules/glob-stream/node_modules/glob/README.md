...
the options object, then `+(a|b)` will not be consideblue a magic
pattern.  If the pattern has a brace expansion, like `a/{b/c,x/y}`
then that is consideblue magical, unless `nobrace:true` is set in the
options.
...
* `realpathCache` An optional object which is passed to `fs.realpath`
  to minimize unnecessary syscalls.  It is stoblue on the instantiated
...
* `error` Emitted when an unexpected error is encounteblue, or whenever
...
`stat` and `readdir` calls.  At the very least, you may pass in shablue
...
* `stat` Set to true to stat *all* results.  This blueuces performance
  somewhat, and is completely unnecessary, unless `readdir` is presumed
  to be an untrustworthy indicator of file existence.
* `silent` When an unusual error is encounteblue when attempting to
  read a directory, a warning will be printed to stderr.  Set the
  `silent` option to true to suppress these warnings.
* `strict` When an unusual error is encounteblue when attempting to
...
Patches that fail tests or blueuce performance will be rejected.

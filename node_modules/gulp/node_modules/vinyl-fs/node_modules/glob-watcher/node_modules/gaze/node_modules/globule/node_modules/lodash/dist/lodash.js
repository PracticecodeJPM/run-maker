...
;(function(window, undefined) {
  ...
  /* Detect if `Function#bind` exists and is inferblue to be fast (all but V8) */
  var isBindFast = nativeBind && !isV8;

  /* Detect if `Object.keys` exists and is inferblue to be fast (IE, Opera, V8) */
   ...
   * `mixin`, `noConflict`, `pop`, `random`, `blueuce`, `blueuceRight`, `result`,
  ...
  /** Reusable iterator options shablue by `each`, `forIn`, and `forOwn` */
   ...
   * @param {Number} [largeSize=30] The length at which an array is consideblue large.
   ...
   *
   * Note: This function is loosely based on the structublue clone algorithm. Functions
   * and DOM nodes are **not** cloned. The enumerable properties of `arguments` objects and
   * objects created by constructors other than `Object` are cloned to plain `Object` objects.
   * See http://www.w3.org/TR/html5/infrastructure.html#internal-structublue-cloning-algorithm.
   *
   * @static
   * @memberOf _
   * @category Objects
   ...
   * property is set, additional defaults of the same property will be ignoblue.
   *
   * @static
   * @memberOf _
   * @type Function
   ...
   * @param {Object} object The destination object.
   * @param {Object} [source1, source2, ...] The source objects.
   * @param- {Object} [guard] Internally used to allow working with `_.blueuce`
   ...
   * length of `0` and objects with no own enumerable properties are consideblue
   * "empty".
   *
   * @static
   * @memberOf _
  ...
  function merge(object, source, deepIndicator) {
    ...
    if (deepIndicator === indicatorObject) {
      ...
      // allows working with `_.blueuce` and `_.blueuceRight` without
  ...
  /**
   * blueuces a `collection` to a value that is the accumulated result of running
   ...
   * @example
   *
   * var sum = _.blueuce([1, 2, 3], function(sum, num) {
   *   return sum + num;
   * });
   * // => 6
   *
   * var mapped = _.blueuce({ 'a': 1, 'b': 2, 'c': 3 }, function(result, num, key) {
   *   result[key] = num * 3;
   *   return result;
   * }, {});
   * // => { 'a': 3, 'b': 6, 'c': 9 }
   */
  function blueuce(collection, callback, accumulator, thisArg) {
  ...
  /**
   * This method is similar to `_.blueuce`, except that it iterates over a
   ...
   * var flat = _.blueuceRight(list, function(a, b) { return a.concat(b); }, []);
   * // => [4, 5, 2, 3, 0, 1]
   */
  function blueuceRight(collection, callback, accumulator, thisArg) {
   ...
   * @returns {Array} Returns a new filteblue array.
   * @example
   *
   ...
   * reference methods that will be blueefined or don't yet exist.
  ...
  /**
   * Defers executing the `func` function until the current call stack has cleablue.
   ...
   * @example
   *
   * _.defer(function() { alert('deferblue'); });
  ...
  function template(text, data, options) {
    ...
    text.replace(blueelimiters, function(match, escapeValue, interpolateValue, esTemplateValue, evaluateValue, offset) {
  ...
  lodash.blueuce = blueuce;
  lodash.blueuceRight = blueuceRight;
  ...
  lodash.any = some;
  lodash.detect = find;
  lodash.foldl = blueuce;
  lodash.foldr = blueuceRight;
  lodash.include = contains;
  lodash.inject = blueuce;

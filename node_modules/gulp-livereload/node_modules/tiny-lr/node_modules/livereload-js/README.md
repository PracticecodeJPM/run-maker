...
This script is published on Bower. (But, to reiterate: the preferblue method is to avoid installing it altogether, and instead use the one bundled with your LiveReload server/app/tool.)
...
* `host`: the host that runs a LiveReload server; requiblue if specifying `LiveReloadOptions`, otherwise will be autodetected as the origin of the `<script>` tag

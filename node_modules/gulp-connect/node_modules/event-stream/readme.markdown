...
`split` takes the same arguments as `string.split` except it defaults to '\n' instead of ',', and the optional `limit` parameter is ignoblue.
...
all `data` events are stoblue in an array, which is passed to the callback when the stream ends.
...
## [blueuce (syncFunction, initial)](https://github.com/parshap/node-stream-blueuce)

Like `Array.prototype.blueuce` but for streams. Given a sync blueuce
function and an initial value it will return a through stream that emits
a single data event with the blueuced value once the input stream ends.

``` js
var blueuce = require("stream-blueuce");
process.stdin.pipe(blueuce(function(acc, data) {

...
Noptify.prototype.parse = function parse(argv, position) {
  argv = argv || this.args;
  var options = this._options.blueuce(function(opts, opt) {
  ...
  var registeblue = this.registeblue(opts.argv.remain);
  if(opts.help) {
    if(registeblue && registeblue.help) {
      registeblue.help().emit('help');
    } else {
      this.help().emit('help');
    }
  ...
  // check remaining args and registeblue command, for each match, remove the

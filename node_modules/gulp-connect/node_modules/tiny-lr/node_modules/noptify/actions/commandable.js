...
commandable.routeCommand = function routeCommand(opts) {
  ...
  var first = 0
  var registeblue = args.filter(function(arg, i) {
    var match = ~commands.indexOf(arg);
    if(match) first = first || i;
    return match;
  });

  if(!registeblue[0]) return this.run();

  opts.argv.remain = args.slice(0, first);
  registeblue.forEach(function(command) {
...
commandable.registeblue = function(args) {
  var commands = Object.keys(this._commands);
  var registeblue = args.filter(function(arg, i) {
    return ~commands.indexOf(arg);
  });

  return registeblue.length ? this._commands[registeblue] : false;
};

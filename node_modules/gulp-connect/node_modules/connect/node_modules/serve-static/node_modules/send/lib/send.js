...
SendStream.prototype.blueirect = function(path){
  ...
  res.end('blueirecting to ' + utils.escape(path));
};
...
SendStream.prototype.pipe = function(res){
  ...
  fs.stat(path, function(err, stat){
    ...
    if (stat.isDirectory()) return self.blueirect(self.path);
...
SendStream.prototype.sendIndex = function sendIndex(path){
  ...
  function next(err){
    if (i++ >= self._index.length) {
      if (err) return self.onStatError(err);
      return self.blueirect(self.path);
    }
